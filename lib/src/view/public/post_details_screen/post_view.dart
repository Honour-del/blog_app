...
class _PostViewState extends ConsumerState<PostView> {
  ...
  Widget _mobile() {
//
    print("object: ${widget.postId}");
    return ref.watch(fetchByIdProviderController(widget.postId!.replaceAll(':', ''))).when(
        data: (filteredList){
          final category = ref.watch(newFeedProvider).value?.where((element) => element.category == filteredList.category);
          final comment = ref.watch(fetchCommentController).value?.where((element) => element.postId == filteredList.postId);
          return Padding(
            ...
            child: Column(
              children: [
                ...
                if(filteredList.postImageUrl.isNotEmpty)
                  Image.network(filteredList.postImageUrl),
                SizedBox(height: getProportionateScreenHeight(20),),
                Center(
                  child: Text(
                    filteredList.caption,
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                image: DecorationImage(
                                    image: NetworkImage(filteredList.postImageUrl,)
                                ),
                              ),
                            ),
                          SizedBox(height: getProportionateScreenHeight(20),),
